...
class MaskEditorDialog(context: Context, overlayManager: OverlayManager) :
    ...
    override fun createView(): View {
        ...
        view.bindClickByNames("btn_save_mask", "btnSaveMask", "btn_save", "btnSave") {
            val svc = MyAutoClickService.instance
            if (svc != null) {
                svc.templateRepository.recalibrateTemplate(0)
                context.vibrateFeedback()
                logDiagnostic("AI_SCANNER", "Маска отредактирована и перекалибрована.")
            }
            hide()
        }

        view.bindClickByNames("btn_cancel_mask", "btnCancelMask", "btn_cancel", "btnClose") {
            hide()
        }
