...
class ScriptsDialog(private val service: MyAutoClickService) {
    ...
    fun show() {
        ...
        fun updateScriptHeaderUI() {
            btnInfinite?.text = if (service.isGlobalScriptInfinite)
                "Бесконечный цикл всего сценария: [ ВКЛ ]"
            else
                "Бесконечный цикл всего сценария: [ НЕТ ]"

            btnInfinite?.backgroundTintList = ColorStateList.valueOf(
                service.getColor(
                    if (service.isGlobalScriptInfinite) R.color.accent_blue else R.color.bg_dark_blue
                )
            )

            btnSelectRelay?.text =
                if (service.globalRelayNextScript.isNotEmpty())
                    "🔗 Эстафета: [ ${service.globalRelayNextScript} 📁 ]"
                else
                    "🔗 Эстафета: [ Выбрать следующий сценарий 📁 ]"

            btnSelectRelay?.backgroundTintList = ColorStateList.valueOf(
                service.getColor(
                    if (service.globalRelayNextScript.isNotEmpty()) R.color.accent_blue else R.color.bg_dark_blue
                )
            )
        }
        ...
        btnCloseScripts?.setOnClickListener {
            service.overlayManager.safeRemoveView(dialogView)
        }
